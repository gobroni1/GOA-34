...
openBtn.addEventListener("click", ()=> {
    ...
    addtask.addEventListener("click", () => {
        ...
        console.log(newdiv.querySelector("h2").textContent);
    });
...
closeBtn2.addEventListener("click", () => {
    ...
    function ATTL(ull) {
        let ls = [];  // Initialize an empty array to hold the text content
    
        // Ensure that ull is a single <ul> element, not a NodeList
        if (ull && ull instanceof HTMLElement) {
            // Get all <li> elements inside the <ul> (ull)
            let listItems = ull.getElementsByTagName("li");
    
            // Iterate over each <li> item
            for (let i = 0; i < listItems.length; i++) {
                ls.push(listItems[i].textContent); // Add the text content to ls as a string
            }
        }
    
        console.log(ls); // Output the array containing text content of all <li> items
    }
    console.log(ATTL(selectedDiv.querySelectorAll("ul")));
});
